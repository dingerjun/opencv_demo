...
int initdevice(void)
{
	...
	LOGI("v4l2_buf_type(CAPTURE=1)    :%d\n",fmt.type); /* CAPTURE=1*/
	LOGI("width              :%d\n",fmt.fmt.pix.width);
	LOGI("height             :%d\n",fmt.fmt.pix.height);
	LOGI("pixel'yuyv'             :%d\n",('Y'|'U'<<8|'Y'<<16|'V'<<24));
	LOGI("pixel'jpeg'         :%d\n",('J'|'P'<<8|'E'<<16|'G'<<24));
	LOGI("pixelformat              :%d\n",fmt.fmt.pix.pixelformat);
	LOGI("v4l1_filed(FIELD_INTERCACED=4) :%d\n",fmt.fmt.pix.field);
	LOGI("bytesperline         :%d\n",fmt.fmt.pix.bytesperline);
	LOGI("sizeimage         :%d\n",fmt.fmt.pix.sizeimage);
	LOGI("colorspace(COLOSPACE_JPEG=7)    :%d\n",fmt.fmt.pix.colorspace);
	LOGI("priv             :%d\n",fmt.fmt.pix.priv);
...
void processimage (const void *p)
{
		...
		LOGI("[colin] After yuyv422toABGRY");

		// matching
		oclbm.process((unsigned char*)rgb);
		LOGI("[colin] After oclbm.process");

		// output
		oclbm.getResult((unsigned char*)rgb,depth,depthSize);
		LOGI("[colin] After oclbm.getResult, depthsize:%d", depthSize);
		//oclbm.getDispImage((unsigned char*)rgb);
}

int readframe(void)
{
	LOGI("[colin] read image");
	struct v4l2_buffer buf;
	unsigned int i;
	int readed = 0;
...
Java_com_camera_simplewebcam_CameraPreview_prepareCamera( JNIEnv* env,jobject thiz, jint videoid){
	...
	if(ret != ERROR_LOCAL){
		...
		depth = (float *)malloc(sizeof(float) * (IMG_WIDTH/2*IMG_HEIGHT*3));
	}
